...
type Props = {
  ...
  placeholder?: string;
  /** Locale kept for UI hint only — actual STT is server-side Whisper (hi). */
  lang?: string;
  disabled?: boolean;
  /**
   * Required for the Whisper rebuild — without these the upload+transcribe
   * round-trip can't reach /api/audio. If absent the mic button is hidden.
   */
  sessionId?: string;
  questionId?: string;
  /**
   * Called when the recording has been uploaded + transcribed. Parent
   * stores the audio_path so admin can replay what Whisper transcribed,
   * and audioTranscript so the answer row carries the spoken text too.
   */
...
export function VoiceTextarea({
  ...
  useEffect(() => {
    return () => {
      // Cleanup on unmount (or when prop deps change)
  ...
  const handleStop = useCallback(async () => {
    ...
    try {
      ...
      if (transcript) {
        // Append the new transcript to whatever's already in the textarea —
        // supports typed-then-spoken and multi-press additive UX.
    ...
    <div className="voice-wrap">
      ...
      {supported === true && canRecord && (
        <div className="voice-controls">
          ...
          <span className="muted small voice-lang">
            {phase === 'recording'
              ? 'Pause kar sakte ho — fir bolo, Stop dabake transcribe ho jayega'
              : phase === 'transcribing'
                ? 'Whisper text bana raha hai…'
                : `Whisper (better Hindi) · ${lang}`}
          </span>
        </div>
      )}
      ...
      {supported === true && !canRecord && (
        <p className="muted small">
          Mic input becomes available once the session is started.
        </p>
      )}
